...
// SetFromBytes sets the internal config based on a byte
func (c *Config) SetFromBytes(data []byte) error {
	...
	config, err := convertKeysToSrtings(untypedConfig)
	if err != nil {
		return err
	}
...
func (c *Config) Get(serviceName string) (map[string]interface{}, error) {
	...
	if !ok {
		return nil, fmt.Errorf("base config is not a ,ap")
	}
...
func convertKeysToSrtings(m map[interface{}]interface{}) (map[string]interface{}, error) {
	...
	for k, v := range m {
		...
		if vMap, ok := v.(map[interface{}]interface{}); ok {
			var err error
			v, err = convertKeysToSrtings(vMap)
			if err != nil {
				return nil, err
			}
		}
